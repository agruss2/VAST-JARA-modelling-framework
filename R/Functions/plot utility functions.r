...
"new.graph" <- function ( size = 1, pointsize = 10, colour.type = "blue" ) {
	...
	#### Mimic Splus default palette
  	if ( colour.type == "red" ) {
			...
			"grey46", rev( rainbow( 21, start = 0, end = 0.35 ) ) ) ) 
  	} 
  	if ( colour.type == "blue" ) {
...
######## Define the "reset.par" function. This function resets par parameters to a set of default values, 
######## ignoring non-standard parameters with a default of '.par'. ('.par' is created when new.graph is called)
...
#### type is automatically added as an extension
#### size represents a fraction of the height of an A4 page
#### res is the resolution in 'dpi'. Values above about 500 may cause errors
#### dimensions (width and height) specify 80% of an A4 page (in inches)
"SavePlot" <- function ( filename = NULL, path = DIR$Figures, size = .par$size, 
	pointsize = .par$pointsize, res = 400, type = "jpg" ) {
  		...
  		if ( is.na( Type ) ) {
    			...
    			jpeg( filename = filename, width = width, height = height * size, units = "in", 
				quality = 100, pointsize = pointsize, bg = "white", res = res, restoreConsole = TRUE )
    			replayPlot( this.image )
    			dev.off()
  		} else if ( type == "pdf" ) {
...
######## Define the "makeTransparent" function
